...
a very common task in real-life applications nowadays. Imagine - for example, your users need to add a profile picture, add some images in a blog article… or if you want to use Machine Learning to detect
objects on the uploaded image then, do Instance Segmentation, or just some image processing using OpenCV -  the first step of the way is always to be able to upload an image.
