...
export default async function AdminPage() {
  ...
  return (
    <div>
      <h1>Admin Dashboard - Users & Comments</h1>

      <div>
        <table>
          <thead className="text-left text-sm font-thin">
            <tr className="border-b border-t border-border-color">
              <th className="border-l border-r border-border-color py-2 px-2">User</th>
              <th className="border-r border-border-color py-2 px-2">Email</th>
              <th className="border-r border-border-color py-2 px-2">Joined</th>
              <th className="border-r border-border-color py-2 px-2">Comments</th>
              <th className="py-2 px-2 border-r border-border-color">Last Comment</th>
            </tr>
          </thead>
              ...
              <tr key={user.id} className="border-b border-border-color">
                <td className="border-l border-r border-border-color py-2 px-2">
                  <div>
                    <div>
                      {user.image ? (
                        <Image className="rounded-full" width={40} height={40} src={user.image} alt={user.name || "User"} />
                      ) : (
                        <div>
                ...
                <td className="border-r border-border-color py-2 px-2">{user.email}</td>
                <td className="border-r border-border-color py-2 px-2">{new Date(user.createdAt).toLocaleDateString()}</td>
                <td className="border-r border-border-color py-2 px-2">{user.comments.length}</td>
                      ...
                      <div>
                        {user.comments.slice(0, 1).map((comment) => (
                          <div key={comment.id} className="mb-2">
                            ...
                            <div className="text-xs">{comment.content.length > 100 ? `${comment.content.substring(0, 100)}...` : comment.content}</div>
                          </div>
                        ))}
                        {user.comments.length > 3 && <div className="text-xs text-secondary-text-color mt-2">+{user.comments.length - 3} more comments</div>}
                      </div>
                    ) : (
